...
def build_document_artifact(
    ...
    renderer.
    """
    resolved_quality_mode = normalize_quality_mode(quality_mode)
    ...
    if requested_format == "docx":
        ...
        try:
            ...
            if doc_plan_obj is not None and agentdeck_plan is not None and render_qa is not None:
                ...
                for _ in range(repair_iteration_cap(resolved_quality_mode)):
...
def build_document_preview(prompt: str, answer: str) -> dict | None:
    ...
    if doc_type == "presentation":
        return build_document_artifact(title, answer, doc_type, "pptx", quality_mode=DEFAULT_QUALITY_MODE)
    try:
