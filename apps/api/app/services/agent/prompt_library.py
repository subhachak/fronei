...
from app.db.models import PromptTemplate, SessionLocal
...
def _row_to_spec(row: PromptTemplate) -> PromptSpec:
    return PromptSpec(
        id=row.id,
...
def _row_to_resolved(row: PromptTemplate) -> ResolvedPrompt:
...
def resolve_prompt(
    ...
    *,
    ...
    try:
        ...
        if profile:
            row = (
                query.filter(PromptTemplate.profile == profile)
        ...
        if row and row.status == "active":
            return _row_to_resolved(row)
        row = (
            query.filter(PromptTemplate.profile.is_(None))
            .order_by(PromptTemplate.updated_at.desc())
            .first()
        )
        if row:
...
def list_prompts() -> list[PromptSpec]:
    ...
    try:
        rows = (
            db.query(PromptTemplate)
            .order_by(
                PromptTemplate.agent_id.asc(),
                PromptTemplate.profile.asc(),
                PromptTemplate.status.desc(),
...
def upsert_prompt(spec: PromptSpec) -> PromptSpec:
    db = SessionLocal()
    try:
        row = db.get(PromptTemplate, spec.id)
        now = _now()
        if row is None:
            row = PromptTemplate(id=spec.id, created_at=now)
            db.add(row)
        ...
        if row.status == "active":
            for active in (
                db.query(PromptTemplate)
                .filter(
                    PromptTemplate.agent_id == row.agent_id,
                    PromptTemplate.profile == row.profile,
                    PromptTemplate.status == "active",
...
def seed_defaults() -> dict[str, int]:
    ...
    try:
        for spec in specs:
            if db.get(PromptTemplate, spec.id):
                counts["skipped"] += 1
                continue
            db.add(
                PromptTemplate(
...
def activate_prompt(prompt_id: str) -> PromptSpec | None:
    db = SessionLocal()
    try:
        row = db.get(PromptTemplate, prompt_id)
        if row is None:
...
def rollback_prompt(prompt_id: str) -> PromptSpec | None:
    db = SessionLocal()
    try:
        row = db.get(PromptTemplate, prompt_id)
        if row is None:
...
def default_prompt_specs() -> list[PromptSpec]:
    ...
    from app.services.agent import document_subtree, research_subtree

    return [
        PromptSpec(
            id="agent.research.brief.default",
        ...
        PromptSpec(
            id="agent.research.coverage_contract.default",
        ...
        PromptSpec(
            id="agent.research.lead.default",
        ...
        PromptSpec(
            id="agent.research.reflection.default",
            agent_id="reflection",
        ...
        PromptSpec(
            id="agent.research.synthesis.default",
            agent_id="synthesis",
        ...
        PromptSpec(
            id="agent.research.citation_verifier.default",
        ...
        PromptSpec(
            id="agent.research.repair.default",
            agent_id="repair",
        ...
        PromptSpec(
            id="agent.document.plan.default",
