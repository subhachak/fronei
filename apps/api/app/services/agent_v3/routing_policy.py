...
from app.db.models import AgentV3RoutingDecisionFeedback, AgentV3RoutingSignalCandidate, SessionLocal
...
def record_routing_feedback(
    *,
    ...
    db = SessionLocal()
    try:
        existing = db.get(AgentV3RoutingDecisionFeedback, turn_id)
        if existing:
            return
...
def list_signal_candidates(status: str | None = None, limit: int = 100) -> dict:
    db = SessionLocal()
    try:
        query = db.query(AgentV3RoutingSignalCandidate)
        if status:
            query = query.filter(AgentV3RoutingSignalCandidate.status == status)
        rows = (
            query.order_by(
                AgentV3RoutingSignalCandidate.status.asc(),
                AgentV3RoutingSignalCandidate.confidence.desc(),
                AgentV3RoutingSignalCandidate.support_count.desc(),
                AgentV3RoutingSignalCandidate.updated_at.desc(),
            )
...
def set_signal_candidate_status(candidate_id: str, status: str) -> dict | None:
    ...
    db = SessionLocal()
    try:
        row = db.get(AgentV3RoutingSignalCandidate, candidate_id)
        if not row:
            return None
        row.status = status
...
def _approved_candidate_matches(text: str) -> list[SignalMatch]:
    ...
    try:
        rows = (
            db.query(AgentV3RoutingSignalCandidate)
            .filter(AgentV3RoutingSignalCandidate.status.in_(("approved", "auto_active")))
            .all()
        )
        matches = []
        for row in rows:
...
def _candidate_payload(row: AgentV3RoutingSignalCandidate) -> dict:
    try:
        examples = json.loads(row.example_turn_ids_json or "[]")
    except Exception:
        examples = []
