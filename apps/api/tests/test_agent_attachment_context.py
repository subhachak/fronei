"""Coverage for app/routers/agent.py:_build_conversation_context, the
...
from app.routers.agent import ATTACHMENT_CONTEXT_MAX_CHARS, _build_conversation_context
from app.services.agent.models import TurnRequest


def _request(**overrides) -> TurnRequest:
    return TurnRequest(message="Summarize this.", **overrides)


def test_no_attachment_returns_base_context_unchanged():
    with patch("app.routers.agent.persistence.conversation_context_text", return_value="prior turn history"):
...
def test_attachment_alone_when_no_base_context():
    with patch("app.routers.agent.persistence.conversation_context_text", return_value=""):
        context = _build_conversation_context("u1", "conv1", _request(attachment_context="Invoice total: $4,200"))
