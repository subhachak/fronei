...
def test_agent_model_recommendation_forces_web_fast(monkeypatch):
    ...
    envelopes = _collect_stream(
        runtime,
        TurnRequest(message="Between OpenAI, Anthropic and Google, what models should I use for a general purpose chatbot?"),
    )
...
def test_agent_routing_feedback_creates_candidate(monkeypatch):
    ...
    db = Session()
    try:
        feedback = db.get(RoutingDecisionFeedback, "turn_feedback_1")
        candidates = db.query(RoutingSignalCandidate).all()
...
def test_agent_approved_learned_signal_forces_web_fast(monkeypatch):
    ...
    envelopes = _collect_stream(
        ...
        TurnRequest(message="Please update the special vendor matrix."),
    )
...
def test_agent_clarify_route(monkeypatch):
    ...
    envelopes = _collect_stream(runtime, TurnRequest(message="Research it."))
...
def test_agent_web_search_prefers_tavily_provider(monkeypatch):
    ...
    sources, call = Tools(you_api_key="you-key", tavily_api_key="tavily-key").search_web("query")
...
def test_agent_web_search_falls_back_to_nimble(monkeypatch):
    ...
    sources, call = Tools(you_api_key="you-key", nimble_api_key="nimble-key").search_web("query")
...
def test_agent_api_stream(monkeypatch):
    ...
    try:
        with TestClient(app) as client:
            response = client.post("/turns/stream", json={"message": "Hello from v3"})
...
def test_agent_api_stream_emits_keepalive_during_quiet_work(monkeypatch):
    ...
    Session = _sqlite_session()
    monkeypatch.setattr(persistence, "SessionLocal", Session)
    monkeypatch.setattr(agent_router, "Runtime", lambda: SlowRuntime())
    monkeypatch.setattr(agent_router, "SSE_HEARTBEAT_SECONDS", 0.01)
    app.dependency_overrides[get_current_user_id] = lambda: "u1"
    app.dependency_overrides[get_current_user_is_admin] = lambda: False
    try:
        with TestClient(app) as client:
...
def test_agent_stream_persists_turn_events_tools_and_artifacts(monkeypatch, tmp_path):
    ...
    try:
        with TestClient(app) as client:
            ...
            stored = client.get(f"/turns/{turn_id}")
        ...
        with Session() as db:
            assert db.get(Turn, turn_id).status == "completed"
            assert db.query(Event).filter(Event.turn_id == turn_id).count() >= 4
            tool_names = [
                row.name
                for row in db.query(ToolCall)
                .filter(ToolCall.turn_id == turn_id)
...
def test_agent_workspace_api_is_user_isolated(monkeypatch):
    ...
    try:
        with TestClient(app) as client:
            created = client.post("/workspaces", json={"name": "U1 workspace"})
            assert created.status_code == 200
            workspace_id = created.json()["id"]
            duplicate = client.post("/workspaces", json={"name": "U1 workspace"})
            assert duplicate.status_code == 200
            assert duplicate.json()["name"] == "U1 workspace 2"
            renamed = client.patch(f"/workspaces/{duplicate.json()['id']}", json={"name": "U1 workspace"})
            assert renamed.status_code == 200
            assert renamed.json()["name"] == "U1 workspace 2"
            conversation = client.post(f"/workspaces/{workspace_id}/conversations", json={"title": "Private work"})
            assert conversation.status_code == 200

            u1_list = client.get("/workspaces")
            assert "U1 workspace" in json.dumps(u1_list.json())

            app.dependency_overrides[get_current_user_id] = lambda: "u2"
            app.dependency_overrides[get_current_user_is_admin] = lambda: False
            u2_list = client.get("/workspaces")
            assert u2_list.status_code == 200
            assert "U1 workspace" not in json.dumps(u2_list.json())

        with Session() as db:
            assert db.query(Workspace).filter(Workspace.user_id == "u1").count() == 2
            assert db.query(Conversation).filter(Conversation.user_id == "u1").count() == 1
...
def test_agent_conversation_turns_are_conversation_scoped(monkeypatch, tmp_path):
    ...
    try:
        with TestClient(app) as client:
            workspace = client.post("/workspaces", json={"name": "Workspace"}).json()
            ...
            assert response.status_code == 200
            turns = client.get(f"/conversations/{conversation['id']}/turns?limit=6")
            ...
            app.dependency_overrides[get_current_user_id] = lambda: "u2"
            app.dependency_overrides[get_current_user_is_admin] = lambda: False
            denied = client.get(f"/conversations/{conversation['id']}/turns?limit=6")
...
def test_agent_conversation_context_connects_followup_turns(monkeypatch, tmp_path):
    ...
    try:
        with TestClient(app) as client:
            workspace = client.post("/workspaces", json={"name": "Context workspace"}).json()
            conversation = client.post(
                f"/workspaces/{workspace['id']}/conversations",
...
def test_agent_workspace_context_is_shared_across_conversations(monkeypatch, tmp_path):
    ...
    try:
        with TestClient(app) as client:
            workspace = client.post("/workspaces", json={"name": "Shared workspace"}).json()
...
def test_agent_vague_followup_does_not_import_other_workspace_conversation(monkeypatch, tmp_path):
    ...
    try:
        with TestClient(app) as client:
            workspace = client.post("/workspaces", json={"name": "Research workspace"}).json()
...
def test_agent_deep_research_uses_brief_objective_for_followup_queries():
    ...
    request = TurnRequest(message="perform a deeper research and give me more accurate answer", research_level="deep")
    brief = ResearchBrief(
        objective=(
...
def test_agent_deep_research_requires_confirmation(monkeypatch):
    ...
    envelopes = _collect_stream(
        runtime,
        TurnRequest(message="Do deep research on enterprise AI governance regulations."),
    )
