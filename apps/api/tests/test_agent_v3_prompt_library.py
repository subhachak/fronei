...
from app.db.models import AgentV3PromptTemplate, Base
from app.main import app
from app.routers import admin as admin_router
from app.services.agent_v3 import prompt_library
...
def test_agent_v3_prompt_seed_and_resolve_from_db(monkeypatch):
    ...
    assert resolved.id == "agent_v3.research.synthesis.default"
...
def test_agent_v3_prompt_activate_archives_same_agent_profile(monkeypatch):
    ...
    draft = prompt_library.AgentV3PromptSpec(
        id="agent_v3.research.synthesis.v2",
        agent_id="synthesis",
        profile=None,
        version="2.0.0",
        status="draft",
    ...
    activated = prompt_library.activate_prompt("agent_v3.research.synthesis.v2")

    assert activated is not None
    assert activated.status == "active"
    with Session() as db:
        old = db.get(AgentV3PromptTemplate, "agent_v3.research.synthesis.default")
...
def test_agent_v3_prompt_rollback_restores_archived_version(monkeypatch):
    ...
    prompt_library.upsert_prompt(
        prompt_library.AgentV3PromptSpec(
            id="agent_v3.document.write.v2",
    ...
    prompt_library.activate_prompt("agent_v3.document.write.v2")

    rolled_back = prompt_library.rollback_prompt("agent_v3.document.write.v2")

    assert rolled_back is not None
    assert rolled_back.id == "agent_v3.document.write.default"
...
def test_admin_agent_v3_prompt_endpoints(monkeypatch):
    ...
    try:
        with TestClient(app) as client:
            seed = client.post("/admin/agent-v3/prompts/seed")
            assert seed.status_code == 200
            assert seed.json()["seeded"]["inserted"] >= 10

            listed = client.get("/admin/agent-v3/prompts")
            assert listed.status_code == 200
            ids = {item["id"] for item in listed.json()["prompts"]}
            assert "agent_v3.research.brief.default" in ids

            upsert = client.post(
                "/admin/agent-v3/prompts",
                json={
                    "id": "agent_v3.research.brief.v2",
            ...
            activated = client.post("/admin/agent-v3/prompts/agent_v3.research.brief.v2/activate")
            assert activated.status_code == 200
            assert activated.json()["activated"] == "agent_v3.research.brief.v2"

            rollback = client.post("/admin/agent-v3/prompts/agent_v3.research.brief.v2/rollback")
            assert rollback.status_code == 200
            assert rollback.json()["rolled_back_to"] == "agent_v3.research.brief.default"
    finally:
