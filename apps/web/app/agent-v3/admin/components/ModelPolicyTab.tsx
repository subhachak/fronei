...
export function ModelPolicyTab({ authorizedFetch }: { authorizedFetch: AuthorizedFetch }) {
  ...
  function load() {
    authorizedFetch('/admin/agent-v3/model-policy')
      .then(async response => {
  ...
  async function save() {
    ...
    setStatus('')
    try {
      const response = await authorizedFetch('/admin/agent-v3/model-policy', {
        method: 'PATCH',
        body: JSON.stringify({
          roles: draft,
  ...
  async function reset() {
    ...
    setStatus('')
    try {
      const response = await authorizedFetch('/admin/agent-v3/model-policy/reset', { method: 'POST' })
