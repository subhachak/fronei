...
export function useAgentV3() {
  ...
  async function ensureActiveConversation(seedMessage: string): Promise<string> {
    ...
    if (!workspace) {
      const response = await authorizedFetch('/agent-v3/workspaces', { method: 'POST', body: JSON.stringify({ name: 'Personal workspace' }) })
  ...
  async function createWorkspace() {
    ...
    const response = await authorizedFetch('/agent-v3/workspaces', { method: 'POST', body: JSON.stringify({ name }) })
    if (!response.ok) {
  ...
  async function saveWorkspaceName(workspaceId: string) {
    ...
    const response = await authorizedFetch(`/agent-v3/workspaces/${workspaceId}`, { method: 'PATCH', body: JSON.stringify({ name }) })
    if (!response.ok) {
