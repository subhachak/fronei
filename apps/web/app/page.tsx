...
function exportConversation(title: string, msgs: MessageOut[]) {
                  ...
                  <table className="admin-table">
                    ...
                    <tbody>
                      {usage?.top_users?.slice(0, 5).map((u: any) => (
                        <tr key={u.user_id}><td className="mono">{u.user_id}</td><td>{fmt$(u.cost, 4)}</td><td>{u.requests}</td></tr>
                      ))}
          ...
          <div className="admin-grid">
            <div className="card admin-table-card">
              ...
              <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>User</th><th>Cost</th><th>Requests</th></tr></thead><tbody>
                {usage.top_users?.map((u: any) => <tr key={u.user_id}><td className="mono">{u.user_id}</td><td>{fmt$(u.cost, 4)}</td><td>{u.requests}</td></tr>)}
              </tbody></table></div>
            </div>
            <div className="card admin-table-card">
        ...
        {tab === 'research' && (
          <div className="card admin-table-card">
            ...
            <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>ID</th><th>User</th><th>Status</th><th>Mode</th><th>Evidence</th><th>Query</th></tr></thead><tbody>
              {research.map(r => <tr key={r.id}><td>{r.id}</td><td className="mono">{r.user_id}</td><td>{r.status}</td><td>{r.mode}</td><td>{r.source_count} src · {r.claim_count} claims</td><td>{r.query}</td></tr>)}
            </tbody></table></div>
          </div>
        )}
        ...
        {tab === 'errors' && (
          <div className="card admin-table-card">
            ...
            <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Time</th><th>User</th><th>Model</th><th>Error</th></tr></thead><tbody>
              {errors.map(e => <tr key={e.id}><td>{e.created_at ? fmtTime(e.created_at) : '—'}</td><td className="mono">{e.user_id}</td><td className="mono">{e.selected_model}</td><td>{e.error}</td></tr>)}
            </tbody></table></div>
          </div>
        )}
        ...
        {tab === 'audit' && (
          <div className="card admin-table-card">
            ...
            <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Time</th><th>Admin</th><th>Action</th><th>Target</th></tr></thead><tbody>
              {audit.map(a => <tr key={a.id}><td>{a.created_at ? fmtTime(a.created_at) : '—'}</td><td className="mono">{a.admin_user_id}</td><td>{a.action}</td><td className="mono">{a.target_user_id ?? '—'}</td></tr>)}
            </tbody></table></div>
          </div>
        )}
  ...
  function doExport(e: MouseEvent, conv: ConversationSummary) {
    ...
    if (activeConvId === conv.id) {
      ...
      apiFetch(`/conversations/${conv.id}`)
        .then(r => r.json())
        .then((d: ConversationDetail) => exportConversation(d.title, d.messages))
        .catch(() => {})
    }
  }
